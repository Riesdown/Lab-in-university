...
void cmd_vel_callback(const geometry_msgs::Twist::ConstPtr& msg)
{
    ...
    cmd.nav_mode=cmd.CMD_VEL_MODE;
    cmd.execution_time=1000;//执行时间1000ms
    ROS_INFO("cmd vel-x y z nav_mode frame_id time: %.2f %.2f %.2f %d %d %d",cmd.x,cmd.y,cmd.z,cmd.nav_mode,cmd.frame_id,cmd.execution_time);
}
...
{
    ros::init(argc,argv,"cmd_vel_ctrl_comand");//初始化节点名称
    ...
    ros::Subscriber cmd_vel_sub=n.subscribe("/cmd_vel",100,cmd_vel_callback);//定义导航控制键盘话题订阅者
