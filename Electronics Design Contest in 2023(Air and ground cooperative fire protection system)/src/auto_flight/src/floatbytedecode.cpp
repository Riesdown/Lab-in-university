...

{
    ...
    FloatToByte(&userdata1,nclink_databuf,cnt);//position.x
    cnt+=4;
    FloatToByte(&userdata2,nclink_databuf,cnt);//position.y
    cnt+=4;
    FloatToByte(&userdata3,nclink_databuf,cnt);//position.z
    cnt+=4;
    FloatToByte(&userdata4,nclink_databuf,cnt);//velocity.x
    cnt+=4;
    FloatToByte(&userdata5,nclink_databuf,cnt);//velocity.y
    cnt+=4;
    FloatToByte(&userdata6,nclink_databuf,cnt);//velocity.z
    cnt+=4;
    FloatToByte(&userdata7,nclink_databuf,cnt);//quaternion.x
    cnt+=4;
    FloatToByte(&userdata8,nclink_databuf,cnt);//quaternion.y
    cnt+=4;
    FloatToByte(&userdata9,nclink_databuf,cnt);//quaternion.z
    cnt+=4;//28
    FloatToByte(&userdata10,nclink_databuf,cnt);//quaternion.w
    cnt+=4;//32
    FloatToByte(&quality,nclink_databuf,cnt);//quality
    cnt+=4;//32
...
_serialport_decode Send_Navigation_Ctrl(uint16_t number,float x,float y,float z,uint8_t navmode,uint8_t frameid,uint32_t execution_time)//地面站指令发送移动距离数据
{
  ...
  FloatToByte(&x,nclink_databuf,cnt);
  cnt+=4;
  FloatToByte(&y,nclink_databuf,cnt);
  cnt+=4;
  FloatToByte(&z,nclink_databuf,cnt);
  cnt+=4;
