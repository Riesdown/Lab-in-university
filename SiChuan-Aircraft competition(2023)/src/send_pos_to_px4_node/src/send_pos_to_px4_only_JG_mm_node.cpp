...
nlink_parser :: TofsenseMFrame0  tofsenser_dis;//激光得到的高度
...
void tofsenser_callback(nlink_parser :: TofsenseMFrame0 msg)
{
  tofsenser_dis = msg;
  dis_last =  dis_now ;
  dis_now =  tofsenser_dis.pixels[0].dis;//
...
int main(int argc, char  *argv[])
{
    ...
    ros :: Subscriber tofsenser_sub = nh.subscribe("/nlink_tofsensem_frame0" , 10 ,  tofsenser_callback);//激光数据
    ...
    while(ros :: ok())
    {
         if( (ros::Time::now() - last_request > ros::Duration(1.0/loop_rate)))
         {
            ...
            if(count %(loop_rate/show_rate)  == 0)
            {
                ...
                ROS_INFO("X:   %f       Y:     %f     Z:    %f    \n  YAW:%f   \n Dis : %f ",  pose_from_px4.pose.position.x , pose_from_px4.pose.position.y ,pose_from_px4.pose.position.z , 180*yaw/M_PI , send_dis) ; //将字符转换为字符数组打印,显示打印时间戳
            }
         }
      ros::spinOnce();
    }
